...
##DESeq2
library(DESeq2)
gene_expression_DESeq2 <- function(counts, group){
...
LUAD.Count <- data.table::fread("File/LUAD.Count", data.table = F)
Group <- ifelse(stringi::stri_detect(colnames(LUAD.Count), "0..$"), "Treatment", "Control")
names(Group) <- colnames(LUAD.Count)
LUAD.Diff <- gene_expression_DESeq2(LUAD.Count, Grooup)
...
d_train=dca(Nomogram,Risk,T,N,M,Stage,times=predictTime)
